...
commentRouter.post('/create', tokenCheck, async (req, res, next) => {
...
commentRouter.get('', async (req, res, next) => {
  const contentsUrl = JSON.parse(decodeURIComponent(req.query.contents));
  try {
    const comments = await commentService.getCommentsByContentsUrl(contentsUrl);
...
commentRouter.get('/content/:url', async (req, res, next) => {
  const contentUrl = req.params.url;
  // cp : 댓글 페이지(코멘트가 있는 페이지)
  // comment : 코멘트(댓글)
  // reply : 답글(대댓글)
  const { cp, comment, reply } = req.query;
  try {
    // 해당 컨텐츠의 댓글 갯수 가져오기
 ...
 */
commentRouter.get('/replies/:order/:include?', async (req, res, next) => {
...
// 답글 위치 정보 찾기
commentRouter.get('/position', async (req, res, next) => {
  const { reply } = req.query;
  const url = reply.split('c_')[1];
  const replyCheck = {};
  try {
    // get comment id by url
    const { _id: replyId, replyIn: { _id: commentId, url: commentUrl } = {} } =
...
commentRouter.get('/sequence', async (req, res, next) => {
  const { comment, reply } = req.query;
